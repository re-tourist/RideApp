...
@Composable
fun ActivityRegistrationScreen(navController: NavController, activityId: Int = 0, onBack: () -> Unit = { navController.popBackStack() }) {
        ...
        content = {
            Column(
                ...
                Column(
                    ...
                    Spacer(modifier = Modifier.height(16.dp))
                    Button(
                        onClick = { navController.navigate("${AppRoutes.ADD_REGISTRATION_CARD}/${activityId}") },
                        modifier = Modifier
                            .fillMaxWidth()
                            .height(56.dp)
                    ) {
                ...
                Column(
                    ...
                    Button(
                        onClick = {
                            ...
                            Thread {
                                val sql = """
                                    INSERT INTO user_activities (user_id, activity_id, relation, status, notes)
                                    VALUES (?, ?, 'registered', 'upcoming', '报名成功')
                                    ON DUPLICATE KEY UPDATE 
                                        status = VALUES(status),
                                        notes = VALUES(notes)
                                """.trimIndent()
                                val result = DatabaseHelper.executeUpdate(sql, listOf(userId, activityId))
                                handler.post {
                                    if (result >= 0) {
                                        Toast.makeText(context, "报名成功", Toast.LENGTH_SHORT).show()
                                        navController.navigate("${com.example.rideflow.navigation.AppRoutes.ACTIVITIES}?category=0")
                                    } else {
